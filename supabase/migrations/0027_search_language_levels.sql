-- 0027: surface REAL teacher language proficiency to the listing card + phone
-- public profile, sourced from the teacher_profiles.languages jsonb column that
-- registration (Page 1 "Languages Spoken") already writes as [{language,
-- proficiency}] via teacherRegistrationApi.submitTeacherProfile().
--
-- Design: the existing `languages` return column stays a PLAIN STRING array
-- (it backs search_facets.languages, the string-based client filters in
-- searchReducer/LanguageFilter/ActiveFiltersDisplay, and RPC ranking via
-- _facet_overlap). We ADD one new display-only column `language_levels` =
-- teacher_profiles.languages [{language, proficiency}]. Nothing else changes;
-- the argument signature is identical, so grants + every caller keep working.
-- Adding a column to RETURNS TABLE requires DROP FUNCTION first (Postgres can't
-- CREATE OR REPLACE a changed return type). Reversible: 0027_..._down.sql.
...
RETURNS TABLE (
  ...
  languages       jsonb,
  language_levels jsonb,          -- NEW: real [{language, proficiency}] for display
  availability    jsonb,
  location        text,
  country         text,
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(tp.languages, '[]'::jsonb)                            AS language_levels,  -- NEW
